...
struct WayPoints my_waypoints_list[total_waypoints];

void wgs2utm(double lat, double lon, int zone , double& east, double& north){
  double lat_rad = lat * M_PI/180; double lon_rad = lon * M_PI/180;
...
void startPoseCallback(const geometry_msgs::Pose2D& msg){
  my_target_pose_start.y = msg.y; my_target_pose_start.x = msg.x;
  my_target_pose_start.theta = msg.theta;	
}

void targetPoseCallback(const geometry_msgs::Pose2D& msg){
	my_target_pose_goal.y = msg.y; my_target_pose_goal.x = msg.x;
	my_target_pose_goal.theta = msg.theta;	
}

void target_goal_id_Callback(const std_msgs::Int16::ConstPtr& msg){ target_goal_id = msg->data; }

// void init_waypoint(void){
//   FILE *fp;
// 	int result = -10;
// 	fp = fopen(WAYPOINT_DIR,"r"); 
...
void init_waypoint(void){
    ...
    fp = fopen(WAYPOINT_DIR, "r");  // 파일 열기
    
    if (fp == NULL) {
        ROS_ERROR("Waypoints file not found: %s", WAYPOINT_DIR);
        return;
    }

    // 첫 번째 줄에서 total_waypoints 값을 읽기
    result = fscanf(fp, "%d", &total_waypoints);
    if (result != 1 || total_waypoints <= 0) {
        ROS_ERROR("Invalid waypoint file format: First line should contain the number of waypoints.");
        fclose(fp);
        return;
    }

    // 동적으로 웨이포인트 리스트 생성
    std::vector<WayPoints> my_waypoints_list(total_waypoints);

    // 웨이포인트 데이터 읽기
    for (int i = 0; i < total_waypoints; i++) {
        result = fscanf(fp, "%lf %lf", &my_waypoints_list[i].x, &my_waypoints_list[i].y);
        if (result != 2) {
            ROS_ERROR("Waypoint data is incomplete. Expected %d points but file ended early.", total_waypoints);
            fclose(fp);
            return;
        }
    }
    ...
    ROS_INFO("Loaded %d waypoints from %s", total_waypoints, WAYPOINT_DIR);
    
    // 디버깅용으로 웨이포인트 출력
    for (int i = 0; i < total_waypoints; i++) {
        ROS_INFO("Waypoint-%d: [%.6lf, %.6lf]", i + 1, my_waypoints_list[i].x, my_waypoints_list[i].y);
    }
}
