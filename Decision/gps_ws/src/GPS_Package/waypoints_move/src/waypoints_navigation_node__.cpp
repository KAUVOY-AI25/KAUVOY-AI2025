	...
	#define MAX_R_STEER 24 // 30 에서 24로 변경
	...
	#define WayPoint_X_Tor 1//0.4
	#define WayPoint_Y_Tor 1//0.5
	...
	// 새로운 waypoint 갱신을 위한 콜백 함수
	void keyboard_input_Callback(const std_msgs::Int16& msg) {
    if (msg.data == 1) {
        // waypoint 갱신 로직 추가
        if (current_waypoint_id < no_waypoints - 1) {
            current_waypoint_id++; // 다음 waypoint로 이동
        } else {
            current_waypoint_id = 0; // 마지막 waypoint 이후 처음으로 돌아감
        }
        ROS_INFO("Current waypoint updated to: [%lf, %lf]", my_waypoints_list[current_waypoint_id].x, my_waypoints_list[current_waypoint_id].y);
    }
}
	...
	void init_waypoint_region(void)
	{
		...
		fp= fopen("/home/kauvoy2/dy_ws/src/pre1.txt","r");
	...
	{	
		...
		//printf("%d %d \n", wp_go_id, no_waypoints);
			...
			ros::Subscriber sub_keyboard_input = n.subscribe("/keyboard_input_topic", 10, &keyboard_input_Callback);

    		ros::spin(); // 콜백을 처리하는 루프 실행
