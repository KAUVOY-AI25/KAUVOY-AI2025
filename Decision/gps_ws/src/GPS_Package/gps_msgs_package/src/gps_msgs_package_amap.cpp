...
geometry_msgs::Pose2D utm2;  //Rear GPS utm 


std_msgs::Float32 gps_heading_angle;
std_msgs::Float32 gps_heading_angle_degree;
...
void navfix_gps2_Callback(const sensor_msgs::NavSatFix::ConstPtr& gps_msg)   // rear GPS
{
	...
	{
		ROS_INFO("GPS Rear Simulation");
		 if(use_two_gps == 1)   fix2 = 2;
	}
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher angle_degree_pub = n.advertise<std_msgs::Float32>("/gps_heading_angle_degree",1); 
  ...
  {	
	...
	if((fix1 == 2) && (fix2 == 2) )  // if 2 gps's are all fixed
	{
       ...
       if(use_two_gps == 1)      ROS_INFO("Rear  GPS E: %.7lf N: %.7lf",Pose2.x,Pose2.y);
