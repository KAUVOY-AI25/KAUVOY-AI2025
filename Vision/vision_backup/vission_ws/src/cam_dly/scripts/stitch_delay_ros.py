...
class PanoramaStitcher:
    def __init__(self):
        ...
        self.center_cap = cv2.VideoCapture(4)
        ...
        # 현재 파일이 위치한 디렉터리 기준으로 절대 경로 설정
        base_dir = os.path.dirname(os.path.abspath(__file__))  # 🔴 scripts 폴더의 절대 경로

        # 호모그래피 행렬 로드 (절대 경로 사용)
        self.H_left_center = self.load_homography(os.path.join(base_dir, "homography_left_center.npy"))
        self.H_center_right = self.load_homography(os.path.join(base_dir, "homography_center_right.npy"))
