...
def Traffic_Light_callback(msg):
    global Traffic_Light  # Add global declaration
    if not msg.channels:
        return None
    ...
    for channel in msg.channels:
        if channel.name in traffic_light_states:
            Traffic_Light = traffic_light_states[channel.name]  # Assign to global variable
            ...
            return Traffic_Light  # 반환

def Traffic_Sign_callback(msg):
    global Traffic_Sign  # Add global declaration
    if not msg.channels:
        return None
    ...
    for channel in msg.channels:
        if channel.name in traffic_sign_states:
            Traffic_Sign = traffic_sign_states[channel.name]  # Assign to global variable
            #print(f"Traffic_Sign: {Traffic_Sign} {channel.name}")
            #print("Confidence Score:", channel.values[0])
            #print("Bounding Box Coordinates (x1, y1, x2, y2):", channel.values[1:5])
            return Traffic_Sign  # 반환
...
def Mission_callback(Traffic_Sign, Traffic_Light):
    ...
    if Traffic_Sign == TRAFFIC_SIGN.LEFT_SIGN and Traffic_Light == TRAFFIC_SIGN.INTER_SIGN:
        mission_num = MISSION.TURN_LEFT_INTER

    elif Traffic_Sign == TRAFFIC_SIGN.LEFT_SIGN and Traffic_Light != TRAFFIC_SIGN.INTER_SIGN:
...
def main_loop():
    ...
    while not rospy.is_shutdown():
        try:
            mission_num = Mission_callback(Traffic_Sign, Traffic_Light)
            mode_num = Mode_callback(mission_num)
            print('status', status, 'Mission_num', mission_num, 'mode', mode)
