...
class PanoramaStitcher:
    def __init__(self):
        ...
        self.H_left_center = self.load_homography("homography_inverted_left_center.npy")
    ...
    def stitch_panorama(self):
        ...
        stitched_right = cv2.warpPerspective(right_image, self.H_center_right, (width_center_right, height_center_right))
        stitched_right[0:center_image.shape[0], 0:center_image.shape[1]] = center_image

        # 왼쪽 이미지를 가운데로 변환 후 병합 (역행렬 적용)
        height_final = max(left_image.shape[0], stitched_right.shape[0])
        width_final = left_image.shape[1] + stitched_right.shape[1]
        H_center_to_left = np.linalg.inv(self.H_left_center)  # 역행렬 사용
        stitched_final = cv2.warpPerspective(left_image, H_center_to_left, (width_final, height_final))
        stitched_final[0:stitched_right.shape[0], left_image.shape[1]:] = stitched_right

        # 최종 파노라마 출력
        cv2.imshow("Panorama", stitched_final)
        cv2.waitKey(1)
