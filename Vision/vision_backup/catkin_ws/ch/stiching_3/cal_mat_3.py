...
class HomographySaver:
    def __init__(self):
        ...
        # 카메라 이미지 구독
        self.left_sub = rospy.Subscriber('/camera3/usb_cam3/image_raw', Image, self.left_image_callback)
        self.center_sub = rospy.Subscriber('/camera2/usb_cam2/image_raw', Image, self.center_image_callback)
        self.right_sub = rospy.Subscriber('/camera1/usb_cam1/image_raw', Image, self.right_image_callback)
    ...
    def process_images(self):
        ...
        if elapsed_time < self.calibration_time:
            ...
            if self.best_H1 is not None and self.best_H2 is not None:
                with open(self.h1_path, 'wb') as f:
                    pickle.dump(self.best_H1, f)
                with open(self.h2_path, 'wb') as f:
                    pickle.dump(self.best_H2, f)
    ...
    def calibrate_homography(self, src_image, dst_image, label):
        """ 중앙 카메라 기준으로 src_image를 dst_image에 맞추는 호모그래피를 계산 """
        try:
